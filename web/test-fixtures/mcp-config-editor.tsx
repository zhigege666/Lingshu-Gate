...
api.credentials = async () => [{ id: "code_graph_token", name: "Code Graph", description: "测试引用", value_masked: "***", created_at: "", updated_at: "" }]
api.validateConfig = async () => ({ ok: true, can_apply: true, summary: { errors: 0, warnings: 0, info: 0, ok: 1 }, checks: [] })
const example = { id: "code_graph", name: "Code Graph", enabled: true, launch: { type: "external" }, transport: { type: "streamable_http", endpoint: "https://graph.example.com/mcp", headers: { Authorization: "Bearer ${credential:code_graph_token}", "X-Client": "lingshu-gate" } }, timeout_seconds: 30, roots: ["/workspace/projects"], analysis: { nested: { enabled: false, count: 0 } } }
function Fixture() {
  const [value, setValue] = useState(JSON.stringify(example, null, 2))
  const [open, setOpen] = useState(true)
  const [saved, setSaved] = useState("")
  const t: TFunction = key => translate("zh-CN", key)
  return <ConsoleDesignProvider><PreviewTheme /><ConfigsPage locale="zh-CN" t={t} configs={[{ id: "code_graph", path: "/example/code_graph.yaml", format: "yaml", manifest: example }]} configErrors={[]} selectedConfigId="code_graph" configText={value} busy={false} editorOpen={open} onCloseEditor={() => setOpen(false)} onNewConfig={() => setOpen(true)} onReloadConfigs={() => {}} onEditConfig={() => setOpen(true)} onApplyConfig={() => {}} onDeleteConfig={() => {}} onConfigTextChange={setValue} onSaveConfig={async next => { setSaved(next || value); setOpen(false) }} /><output aria-label="测试保存结果">{saved}</output></ConsoleDesignProvider>
}
