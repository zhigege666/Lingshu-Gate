  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        toolbar={<PageToolbar query={query} onQueryChange={setQuery} placeholder={`${t("search")} ${t("check")} / ${t("detail")}`} resultCount={filteredChecks.length} resultLabel={t("check")} clearLabel={t("clearSearch")} />}
        stats={[
  ...
  return (
    <Card>
      <CardContent className="flex flex-col gap-3 p-3 md:p-4">
        ...
        <details>
          <summary className="cursor-pointer text-sm font-medium">{t("runtimeEnvironment")}</summary>
          <div className="mt-3 flex flex-col gap-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="text-sm text-muted-foreground">{t("runtimeEnvironmentDesc")}</p>
              <Button size="sm" variant="secondary" onClick={() => void refresh()}>{t("refresh")}</Button>
            </div>
            {env ? <>
            ...
            </> : !error ? <p className="text-sm text-muted-foreground">{t("waiting")}</p> : null}
          </div>
        </details>
      </CardContent>
    </Card>
